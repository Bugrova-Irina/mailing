...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS("Разрешение добавлено в группу managers")
            )
        except Permission.DoesNotExist:
            self.stdout.write(
                self.style.ERROR('Разрешение "can_disable_user" не найдено')
            )
        ...
        for user_data in users_data:
            ...
            if user_data.get("is_manager", False):
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Пользователь {user.email} добавлен в группу managers"
                    )
                )
