...
from sending_messages.forms import (
    AttemptCreateForm,
    LetterCreateForm,
    MailingCreateForm,
    RecipientsCreateForm,
)
from sending_messages.models import AttemptToSend, Letter, Mailing, MailingListRecipient
...
class MailingListRecipientUpdateView(UpdateView):
    ...
    def get_success_url(
        ...
        return reverse(
            "sending_messages:recipient_detail", args=[self.kwargs.get("pk")]
        )
...
class AttemptToSendListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Получаем статистику
        context["success_attempts"] = AttemptToSend.objects.filter(
            status=AttemptToSend.SUCCESS
        ).count()
        context["wrong_attempts"] = AttemptToSend.objects.filter(
