...
from sending_messages.models import AttemptToSend, Mailing
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        updated_completed = Mailing.objects.filter(
            status=Mailing.STARTED, end_sending__lt=current_time
        ).update(status=Mailing.COMPLETED)

        self.stdout.write(
            f"Обновлено статусов завершенных рассылок: {updated_started}.\nОбновлено статусов завершенных рассылок: {updated_completed}."
        )
        ...
        for mailing in active_mailings:
            ...
            if not total_recipients:
                self.stdout.write(
                    self.style.WARNING("  X Нет получателей для рассылки")
                )
                continue
            ...
            for recipient in recipients:
                ...
                try:
                    ...
                    attempt.save()
                    self.stdout.write(
                        self.style.ERROR(f"  X [Ошибка] {recipient_email}: {error_msg}")
                    )

            # Статистика по рассылке
            success_rate = (
                (success_count / total_recipients) * 100 if total_recipients else 0
            )
            self.stdout.write(
                self.style.SUCCESS(
                    f"  ИТОГ: Отправлено {success_count}/{total_recipients} "
                    f"({success_rate:.1f}%)"
                )
