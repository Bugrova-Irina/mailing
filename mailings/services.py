...
from .models import AttemptToSend, Letter, Mailing, Recipients
...
def get_cached_recipients(user):
    ...
    if not CACHE_ENABLED:
        return (
            Recipients.objects.all()
            if user.groups.filter(name="managers").exists()
            else Recipients.objects.filter(owner=user)
        )
...
def get_cached_letters(user):
    ...
    if not CACHE_ENABLED:
        return (
            Letter.objects.all()
            if user.groups.filter(name="managers").exists()
            else Letter.objects.filter(owner=user)
        )
...
def get_cached_mailings(user):
    ...
    if mailings is None:
        mailings = list(
            Mailing.objects.all()
            if user.is_superuser or user.has_perm("users.can_disable_user")
            else Mailing.objects.filter(owner=user)
        )
...
def get_cached_attempts(user):
    ...
    if attempts is None:
        attempts = list(
            AttemptToSend.objects.all()
            if user.is_superuser or user.has_perm("users.can_disable_user")
            else AttemptToSend.objects.filter(owner=user)
        )
